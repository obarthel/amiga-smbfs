...
/* Source: ftp://ftp.unicode.org/Public/MAPPINGS/VENDORS/MICSFT/PC/CP850.TXT (2018-05-10) */

const unsigned int cp850_to_unicode[256] =
{
   /*
...
{
	unsigned char unicode_to_cp850[256];
	int i;
	int c;

	memset(unicode_to_cp850,0,sizeof(unicode_to_cp850));

	printf("const unsigned char cp850_to_unicode[256] =\n{\n");

	for(i = 0 ; i < 256 ; i++)
	{
		c = cp850_to_unicode[i];
		if(c > 255)
			c = 0;

		if(c != 0)
			unicode_to_cp850[c] = i;
	...
	printf("const unsigned char unicode_to_cp850[256] =\n{\n");

	for(i = 0 ; i < 256 ; i++)
	{
		c = unicode_to_cp850[i];
